...
class LLM_HLP_Generator:
    ...
    def generate_prompt(self, curr_task, k, removeNav=False, naturalFormat=False, includeLow=False):
        ...
        for retrieved_task in knn_retrieved_examples:
            trainTaskRow = self.knn_set.loc[self.knn_set["task"] == retrieved_task].iloc[0]
            step_list = [literal_eval(listItem) for rowItem in trainTaskRow["gold_traj"] for listItem in rowItem]
            if removeNav:
                step_list = [item for item in step_list if "Navigation" not in item]
            if naturalFormat:
                step_list = [(ACT_TO_STR[item[0]], item[1].lower()) for item in step_list]

            planSplit = random.randint(0, len(step_list) - 1)
        ...
        task_past_plan_str = self.format_plan_str(curr_task["completed_plans"])
        task_obj_str = self.format_object_str(curr_task["vis_objs"])
    ...
    def run_gpt(self, prompt, logit_bias_text, model='gpt-4-turbo', max_tokens=200):
        gpt_output = []
        logit_biases = {}
        tokens = self.tokenizer.encode(logit_bias_text)
        for token in tokens:
            logit_biases[token] = 0.1
    ...
    def generate_hlp(self, curr_task, k):
        ...
        generated_hlp, gpt_output = self.run_gpt(prompt, curr_task["vis_objs"])
        return generated_hlp

    def format_object_str(self, obj_list):
        return ", ".join(obj_list).lower()

    def format_plan_str(self, plan_list):
        if not plan_list:
            return ""
        lowercased_plan_list = [(item[0], item[1].lower()) for item in plan_list]
        plans = [" ".join(item) for item in lowercased_plan_list]
        return ', '.join(plans)

    def format_to_virtualhome(self, plan):
        # Convert the plan into a VirtualHome-compatible format
        # E.g., [('OpenObject', 'Drawer')] -> ['[OpenObject] <Drawer>']
        formatted_plan = []
...
if __name__ == '__main__':
    ...
    # Format the generated plan for VirtualHome compatibility
    formatted_plan = hlp_generator.format_to_virtualhome(generated_plan)
