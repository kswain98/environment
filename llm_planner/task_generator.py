...
def get_llm_response(client, prompt, model):
    try:
        ...
        rate_limits = {
            "remaining_requests": chat_completion.headers.get("x-ratelimit-remaining-requests"),
            "remaining_tokens": chat_completion.headers.get("x-ratelimit-remaining-tokens"),
...
def main(api_key, prompt, model, interval, output_file):
    ...
    while True:
        ...
        if response:
            ...
            response_data = {"timestamp": timestamp, "prompt": prompt, "response": response}
            with open(output_file, "a") as file:
                file.write(json.dumps(response_data) + "\n")
            print("Response appended to JSON file.")
            
            if rate_limits.get("remaining_requests") and int(rate_limits["remaining_requests"]) < 10:
                print("Warning: Approaching daily request limit.")
            if rate_limits.get("remaining_tokens") and int(rate_limits["remaining_tokens"]) < 50:
